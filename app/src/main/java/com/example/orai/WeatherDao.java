...
public class WeatherDao extends SQLiteOpenHelper {
    ...
    // Atsakingas uz lenteliu sukurima
    @Override
    public void onCreate(SQLiteDatabase db) {
        String CREATE_WEATHER_TABLE = "" +
                "CREATE TABLE " + TABLE_NAME + "("
                + KEY_ID + " INTEGER PRIMARY KEY, " + COUNTRY_NAME +
                " TEXT, " + DEGREES + " REAL " + ");";
    ...
    public List<Weather> readAll (){
        ...
        // TODO getDouble neveikia
        if (cursor.moveToFirst()){
           do {
               Weather weather = new Weather();
               weather.setId(Integer.parseInt(cursor.getString(0)));
               weather.setDegrees(String.valueOf(cursor.getDouble(1)));
               weather.setCountryName(cursor.getString(2));
